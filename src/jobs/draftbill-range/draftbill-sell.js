const {RATA_DRAFT_BILL_SELL_RANGED} = require('../queues/queues');
const models = require('../../models/rata');
const _ = require('lodash');
const draftBill = require('../../services/draftbill-ranged.service')

module.exports = () => {
    const scheduler_id = 'RATA_DRAFT_BILL_SELL_RANGED';
    RATA_DRAFT_BILL_SELL_RANGED.process(async(job,done) => {
        try{    
            const {from, to} = job.data;
            const id = job.id
            
            await draftBill.sell(from,to,id)
    ...
    RATA_DRAFT_BILL_SELL_RANGED.on('active',async(job) => {
    ...
    RATA_DRAFT_BILL_SELL_RANGED.on('completed',async(job) => {
    ...
    RATA_DRAFT_BILL_SELL_RANGED.on('failed',async(job,err) => {
        await models.scheduler_auto_sync_trckr_tbl.update({
            job_status: 'FAILED',
